...
CREATE POLICY IF NOT EXISTS "Admins can insert posts" ON public.posts
...
CREATE POLICY IF NOT EXISTS "Admins can update posts" ON public.posts
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE id = auth.uid() AND role IN ('admin', 'moderator')
...
-- Policy per insegnamenti (pubblici per tutti, admin possono modificare)
CREATE POLICY IF NOT EXISTS "Teachings are viewable by everyone" ON public.teachings
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Admins can manage teachings" ON public.teachings
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE id = auth.uid() AND role IN ('admin', 'moderator')
...
SELECT 'Database setup completato! Ora puoi creare l''utente admin.' as status; 